...
export default function EditarInspeccionPage() {
  ...
  return (
    <div>
        ...
        <div key={nombreGrupo} className="section">
          <table className="table excel">
            <thead>
              ...
              <tr className="trow">
                <th className="tcell" style={{ width: 120 }}>Item</th>
                <th className="tcell">Descripcion</th>
                <th className="tcell" style={{ width: 240 }}>Valor</th>
                <th className="tcell" style={{ width: 240 }}>Observacion</th>
              </tr>
            </thead>
            <tbody>
              {items.map((it) => (
                <tr key={it.id} className="trow">
                  ...
                  <td className="tcell">
                    <ToggleTri name={`tri-${it.id}`} value={res[it.id]?.v || ""} onChange={(v)=>setValor(it.id, { v })} />
                  </td>
                  <td className="tcell">
                    <input className="input" placeholder="Observacion" value={res[it.id]?.obs || ""} onChange={(e)=>setValor(it.id, { obs: e.target.value })} />
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="section">
        <h3 className="sectionTitle">Registro fotografico (basico)</h3>
